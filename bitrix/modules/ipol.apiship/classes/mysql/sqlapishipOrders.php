<?
class sqlapishipOrders
{
	public function apishipLog($wat,$sign){apishipdriver::apishipLog($wat,$sign);}
	private static $tableName = "ipol_apiship";
	...
	public static function GetByOI($orderId)
	{
		...
		$orderId=$DB->ForSql($orderId);
		$strSql =
            "SELECT PARAMS, STATUS, apiship_ID, MESSAGE, OK, MESS_ID ".
            "FROM ".self::$tableName." ".
			"WHERE ORDER_ID = '".$orderId."'";
	...
	public static function updateStatus($arParams){
		...
		// if($arParams["apiship_ID"])
			// $setStr.="apiship_ID = '".$arParams["apiship_ID"]."',";
		if($arParams["MESS_ID"])
		...
		$strSql =
			...
			SET ".$setStr."
			WHERE apiship_ID = '".$arParams["apiship_ID"]."'";
	...
	public static function getStatuses($arParams = array())
	{
		global $DB;
